...
규칙: 1)모든 텍스트 반드시 한국어만 사용, 외국어 절대 금지 2)steps는 최소 6~8단계로 각 단계마다 구체적인 시간/온도/양/불 세기를 상세히 설명 3)3~4가지 요리 추천 4)matchPercent는 정수 5)difficulty는 "쉬움"/"보통"/"어려움" 중 하나 6)추천하는 요리들은 서로 완전히 다른 종류여야 함 (예: 파스타류, 볶음류, 국물류, 구이류 등 카테고리가 겹치면 안됨) 7)비슷한 이름이나 비슷한 조리법의 요리를 중복 추천 절대 금지 8)입력된 재료를 활용할 수 있는 다양한 종류의 실제 요리를 추천할 것`;
