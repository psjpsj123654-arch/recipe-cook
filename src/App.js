...
export default function App() {
  ...
  const handleSearch = async () => {
    ...
    const prompt = `당신은 한국인을 위한 요리 전문가입니다. 반드시 모든 텍스트를 한국어로만 작성하세요. 영어, 일본어, 중국어, 아랍어 등 다른 언어는 절대 사용하지 마세요.
...
규칙: 1)실제로 존재하는 요리 이름만 사용하고 추천하세요 2)모든 텍스트 반드시 한국어만 사용 3)steps는 최소 6단계 이상, 각 단계마다 구체적 시간/온도/양 포함 4)5~6가지 요리 추천 5)matchPercent는 정수 6)difficulty는 "쉬움"/"보통"/"어려움" 중 하나 7)각 요청은 이전 요청과 완전히 독립적으로 처리하세요. 이전 입력 재료와 추천 결과는 참고하지 마세요.`;
