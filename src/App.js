...
이전 대화 내용이나 이전에 추천했던 요리는 완전히 무시하세요. 오직 지금 입력된 재료만을 기준으로 판단하세요. 반드시 실제로 존재하고 사람들이 실제로 자주 먹는 요리만 추천하세요. 모든 재료 이름을 요리 이름에 직접 넣지 마세요 (예: "양파 계란 볶음" 같은 이름 금지). 입력된 재료가 메인 재료가 아니어도 되고 부재료로 들어가는 요리도 추천 가능합니다. 예를 들어 계란+양파가 있으면 계란찜, 오믈렛, 순두부찌개, 계란볶음밥 등 실제 요리를 추천하세요.
...
규칙: 1)모든 텍스트 반드시 한국어만 사용, 외국어 절대 금지 2)steps는 최소 6~8단계로 각 단계마다 구체적인 시간/온도/양/불 세기를 상세히 설명 3)3~4가지 요리 추천 4)matchPercent는 정수 5)difficulty는 "쉬움"/"보통"/"어려움" 중 하나 6)실제로 존재하는 요리 이름만 사용`;
